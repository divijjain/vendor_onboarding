defmodule VendorOnboarding.Agent.RunTest do
  use VendorOnboarding.DataCase, async: false

  import VendorOnboarding.AgentFakes

  alias VendorOnboarding.Agent.Checkpoint.Repository
  alias VendorOnboarding.Agent.Run
  ...
  setup do
    ...
    Application.put_env(:vendor_onboarding, :agent_callback_fun, fn payload ->
      Kernel.send(test_pid, {:callback, payload})
      {:ok, :captured}
    end)

    on_exit(fn -> Application.delete_env(:vendor_onboarding, :agent_callback_fun) end)
    :ok
  end
  ...
  test "still reports :ok when the callback function itself errors" do
    ...
    Application.put_env(:vendor_onboarding, :agent_callback_fun, fn _payload ->
