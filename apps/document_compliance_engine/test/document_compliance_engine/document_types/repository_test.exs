defmodule DocumentComplianceEngine.DocumentTypes.RepositoryTest do
  ...
  @valid_attrs %{slug: "test_only_type", name: "Test Only Type"}

  describe "insert/1" do
    test "creates a row on the happy path" do
      assert {:ok, document_type} = Repository.insert(@valid_attrs)
      assert document_type.slug == "test_only_type"
      assert document_type.name == "Test Only Type"
  ...
  describe "get_by_slug/1" do
    test "finds an existing row" do
      ...
      assert %{id: id} = Repository.get_by_slug("test_only_type")
  ...
  describe "list/0" do
    test "includes the migration-seeded type library plus any created since" do
      ...
      assert "test_only_type" in slugs
    end
  end
end
