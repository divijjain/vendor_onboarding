defmodule VendorOnboarding.AgentRuns.Schema.AgentRun do
  @moduledoc """
  One agent run's output for a given onboarding. A separate table (not
  columns bolted onto `onboardings`) so re-runs keep history instead of
  ...
  def create_changeset(agent_run, attrs) do
    agent_run
    |> cast(attrs, [:vendor_onboarding_id, :status])
    |> validate_required([:vendor_onboarding_id, :status])
  end
