defmodule VendorOnboardingWeb.WebhookControllerTest do
  ...
  test "POST /webhooks/vendor_onboarding creates an document_job on the happy path", %{conn: conn} do
    conn = post_webhook(conn, payload())

    assert %{"id" => id, "status" => "received"} = json_response(conn, 201)
    assert {:ok, document_job} = VendorOnboarding.DocumentJobs.get_document_job(id)
  ...
  test "POST /webhooks/vendor_onboarding rejects a request with no signature header", %{
    conn: conn
  } do
    conn =
      conn
      |> put_req_header("content-type", "application/json")
      |> post(~p"/webhooks/vendor_onboarding", payload())

    assert json_response(conn, 401) == %{"error" => "invalid_signature"}
  end

  test "POST /webhooks/vendor_onboarding rejects a request with a wrong signature", %{
    conn: conn
  } do
    conn =
      conn
