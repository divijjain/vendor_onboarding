defmodule VendorOnboarding.DocumentJobs.RepositoryTest do
  ...
  describe "insert/1" do
    test "creates a row with status :received on the happy path" do
      assert {:ok, document_job} = Repository.insert(@valid_attrs)
      assert document_job.status == :received
      assert document_job.idempotency_key == "abc123"
      assert document_job.document_paths == @valid_attrs.document_paths
    end
  ...
  describe "get_by_idempotency_key/1" do
    test "finds an existing row" do
      {:ok, document_job} = Repository.insert(@valid_attrs)
  ...
  describe "get/1" do
    test "returns {:ok, document_job} for an existing id" do
      {:ok, document_job} = Repository.insert(@valid_attrs)
