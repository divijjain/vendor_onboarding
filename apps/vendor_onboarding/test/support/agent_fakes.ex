defmodule VendorOnboarding.AgentFakes do
  @moduledoc """
  ...
  keys are prefixed `agent_*` since they share `:vendor_onboarding`'s
  ...
  def stub(overrides) do
    Enum.each(overrides, fn {key, fun} ->
      Application.put_env(:vendor_onboarding, :"agent_#{key}", fun)
    end)

    ExUnit.Callbacks.on_exit(fn ->
      Enum.each(overrides, fn {key, _fun} ->
        Application.delete_env(:vendor_onboarding, :"agent_#{key}")
      end)
    end)
  end
